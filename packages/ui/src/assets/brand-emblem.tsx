    ...
    >
      <defs>
        <linearGradient id="coffeeGrad" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="var(--accent-amber, #f59e0b)" />
          <stop offset="50%" stopColor="var(--brand-coffee, #9c6b3a)" />
          <stop offset="100%" stopColor="var(--inverse-primary, #5e3f25)" />
        </linearGradient>
      ...
      <circle
        ...
        strokeOpacity="0.35"
        strokeWidth="1.5"
        fill="var(--surface-secondary, #111114)"
      />
      ...
      <path
        ...
        strokeLinecap="round"
      />
      <circle cx="50" cy="78" r="2.5" fill="var(--accent-amber, #f59e0b)" />
    </svg>
  );
};
  ...
  return (
    <div className={cn("flex items-center gap-2.5 select-none", className)} {...props}>
      ...
      <div className="flex flex-col leading-tight">
        <span className="font-bold text-lg tracking-tight text-[var(--text-primary)] font-heading group-hover:text-[var(--primary)] transition-colors">
          Warkop Ya&apos;reh
        </span>
        {showSubtitle && (
          <span className="font-mono text-[11px] tracking-widest text-[var(--text-muted)] uppercase">
            {subtitle}
          </span>
        )}
      </div>
