...
const customerEmail =
  process.env.PHASE6_CUSTOMER_EMAIL ?? 'phase6.proof@warkopyareh.local';
...
invariant(
  inventoryReloaded?.supplier === inventoryMarker.supplier &&
    Number(inventoryReloaded?.stockQuantity) === inventoryMarker.stockQuantity,
...
const branchReloaded = await api(`/branches/${encodeURIComponent(branch.id)}`);
...
delightProof.aurora = await record('aurora-motion', async (page) => {
  ...
  const after = await blob.evaluate((node) => getComputedStyle(node).transform);
...
delightProof.ctaHover = await record('cta-hover', async (page) => {
  ...
  await cta.waitFor();
  const before = await cta.evaluate((node) => getComputedStyle(node).transform);
  await cta.hover();
  await page.waitForTimeout(300);
  const after = await cta.evaluate((node) => getComputedStyle(node).transform);
  invariant(before !== after, 'Primary CTA transform did not change on hover');
  return { before, after };
});
...
const reducedMotionProof = await reducedPage.evaluate(() => {
  ...
  return {
    ...
    counterSettled:
      counterText !== null &&
      counterTarget !== null &&
      Math.abs(Number(counterText.replace(',', '.')) - Number(counterTarget)) <
        0.001,
  };
});
invariant(
...
const report = {
  ...
  scope: 'local development services and PostgreSQL on localhost only',
  api: {
