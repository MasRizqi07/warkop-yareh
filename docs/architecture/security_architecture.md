...
sequenceDiagram
    ...
    A->>R: Store hash(refreshToken) with 7d TTL
    A-->>C: Set-Cookie: __Host-refresh-token (HttpOnly, Secure, SameSite=Strict)
    A-->>C: { accessToken, user }

    Note over C: Access token expires after 15 min
    C->>A: POST /api/v1/auth/refresh (Cookie: __Host-refresh-token)
    A->>R: Verify hash(refreshToken) exists
    alt Token Valid
...
|:------|:----------|:----|:--------|:---------|
| Access Token | RS256 | 15 min | Authorization header | userId, role, branchId, tier |
| Refresh Token | Opaque (cuid) | 7 days | HttpOnly cookie | Random identifier |
...
| `JWT_SECRET` (RS256 private key) | AWS Secrets Manager / Vercel env | 90 days |
| `MIDTRANS_SERVER_KEY` | Environment variable | Per Midtrans policy |
| `NEXTAUTH_SECRET` | Environment variable | 90 days |
...
| A02 | Cryptographic Failures | bcrypt password hashing, HTTPS only, RS256 JWT |
