...
Create separate `staging` and `production` GitHub Environments and store the
same secret names in each environment with environment-specific values. Protect
the `production` environment with required reviewers. The CD jobs bind to the
selected environment, so a staging migration cannot silently reuse production
credentials.
...
2. **Production Deployments (Strictly Guarded)**:
   ...
   - Production deployment requires explicit manual `workflow_dispatch` with:
     ...
     - `service: all`, `api`, `web`, or `admin`
   - API delivery targets Railway; web and admin delivery target their
     respective Vercel production projects.
