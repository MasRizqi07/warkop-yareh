...
export default function MenuPage() {
  ...
  const categories = [{ id: 'all', label: 'All Creations', count: allProducts.length },

    ...(catalog.data?.categories ?? []).map((category) => ({ id: category.slug, label: category.name, count: allProducts.filter((product) => product.category === category.slug).length }))];
  ...
  const fulfillmentModes: Array<{ type: FulfillmentType; label: string; icon: React.ElementType; sub?: string }> = [
    ...
    { type: "dine-in", label: "Dine-In QR", icon: Utensils, sub: tableLabel || "Pindai QR meja" },
  ...
  return (
    ...
    <main className="min-h-screen bg-canvas-obsidian text-text-primary pt-4 pb-32 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        {branches.isError || catalog.isError ? <DataState title="Katalog belum dapat dimuat" retry={() => { void branches.refetch(); void catalog.refetch(); }} /> : branches.isPending || (activeBranch && catalog.isPending) ? <LoadingState label="Memuat menu cabang..." /> : filteredProducts.length === 0 ? (
