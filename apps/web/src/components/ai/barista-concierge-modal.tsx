...
export function BaristaConciergeModal() {
  ...
  const findCatalogProduct = (productId: string) =>
    catalog.data?.products.find((product) => product.id === productId);
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 no-scrollbar">
                {messages.map((m) => {
                  const recommendedProduct = m.recommendation
                    ? findCatalogProduct(m.recommendation.id)
                    : undefined;
                  const snackProduct = m.snackPairing
                    ? findCatalogProduct(m.snackPairing.id)
                    : undefined;
                        ...
                        <div className="mt-3 w-full max-w-[92%] bg-[var(--bg-surface-overlay)] border border-[var(--accent-fill)]/30 rounded-2xl p-4 shadow-md space-y-3 animate-in fade-in zoom-in-95 duration-200">
                          ...
                          {m.snackPairing && (
                            <div className="pt-2 border-t border-[var(--border-default)]/40 flex items-center justify-between">
                              ...
                              <button
                                onClick={() =>
                                  handleAddToCart(m.snackPairing!.id)
                                }
                                disabled={!snackProduct}
                              ...
                              >
                                ...
                                <span>
                                  +Rp{' '}
                                  {snackProduct
                                    ? snackProduct.price.toLocaleString('id-ID')
                                    : 'Tidak tersedia'}
                                </span>
                              </button>
                            </div>
