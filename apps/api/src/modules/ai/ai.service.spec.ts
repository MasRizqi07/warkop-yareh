...
describe('AiService', () => {
  ...
  it('recommend: should infer fruity profile from query', async () => {
    const res = await service.recommend({ userQuery: 'mau kopi yang asem seger strawberry' });
    expect(res.highlightedProducts.some((p) => p.name.includes('Ijen') || p.name.includes('Toraja'))).toBe(true);
  ...
  it('chatWithBarista: should answer sleepiness query with high caffeine bold drinks', async () => {
    const chat = await service.chatWithBarista('lagi ngantuk banget butuh begadang kerja tugas');
  ...
  it('chatWithBarista: should answer non-coffee inquiry with matcha / mocktail', async () => {
    const chat = await service.chatWithBarista('ada minuman non kopi yang enak?');
