    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        if (
          ...
          /reservations_table_no_overlap|23P01/.test(error.message)
        )
          throw new ConflictException(
            'Table is already reserved for this time slot',
          );
  ...
  async updateStatus(
    ...
    return this.prisma.withTenantTransaction(async (tx) => {
      ...
      if (
        ...
        reservation.order.paymentStatus !== 'PAID'
      )
        throw new ConflictException(
      ...
      if (reservation.order && status === ReservationStatus.CANCELLED) {
        if (
          reservation.order.paymentStatus === 'PAID' ||
          (reservation.order.paymentStatus === 'UNPAID' &&
            reservation.order.payment)
        )
          throw new ConflictException(
            'Resolve the payment with the provider before cancelling this booking',
          );
