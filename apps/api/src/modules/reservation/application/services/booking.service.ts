...
export function bookingInterval(packageId: string, date: string) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date))
    throw new BadRequestException('Use YYYY-MM-DD for the booking date');
  ...
  if (
    ...
    startAt.getTime() > Date.now() + 366 * 86400000
  )
    throw new BadRequestException('Choose a future booking within one year');
  const endAt = new Date(startAt.getTime() + selected.durationMinutes * 60000);
  const endTime = new Date(endAt.getTime() + 7 * 3600000)
    .toISOString()
    .slice(11, 16);
...
export class BookingService {
  ...
  async catalog() {
    const products = await this.prisma.product.findMany({
      where: {
        id: { in: [...PACKAGES.map((item) => item.id), ...ADDONS] },
        deletedAt: null,
      },
  ...
  async quote(packageId: string, addonIds: string[] = []) {
    return this.prisma.withTenantTransaction(async (tx) =>
      this.price(tx, packageId, addonIds),
    );
  }
  ...
  async create(userId: string, input: BookingInput, key: string) {
    if (!key || key.trim().length < 8 || key.length > 128)
      throw new BadRequestException(
        'Idempotency-Key must have 8-128 characters',
      );
    const normalized = {
      ...input,
  ...
  private async price(
    ...
    if (
      !PACKAGES.some((item) => item.id === packageId) ||
      addonIds.some((id) => !ADDONS.includes(id)) ||
      new Set(addonIds).size !== addonIds.length
    )
      throw new BadRequestException('Invalid package or add-on selection');
    ...
    if (
      ...
      items.some((item) => !Number.isSafeInteger(item.price) || item.price <= 0)
    )
      throw new BadRequestException('Booking prices are not configured');
    return {
      ...calculateCheckout(items.reduce((sum, item) => sum + item.price, 0)),
      items,
    };
  }
}
